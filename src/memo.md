...
This is a test of whether a general-purpose LLM pipeline — CourtListener for discovery, the Federal Register API for regulatory history, Claude Sonnet 4.6 for the coding decisions — can approximate the empirical method in Bressman & Stack's 2025 piece in a form that a practitioner could actually assemble in an afternoon. It can approximate it. And the places where it diverges from the paper's counts turn out to track largely the design features of the paper's coding workflow — dual review, subsection-specific reading, a strict "wholly inconsistent" threshold for reversal, and basic data hygiene — that distinguish careful legal coding from a single-pass classifier. When those features are layered onto the pipeline in computational form, the pipeline's numbers converge toward the paper's.

## Headline numbers

After the full pipeline and five design-adherence passes:

<ul>
  <li><strong>114 step-two affirmances</strong> of notice-and-comment rules (paper: 96).</li>
  <li><strong>11 reversal-coded amendments</strong> across <strong>5 unique affirmed rules</strong> (paper: 3 rules).</li>
  <li>Both of the paper's named D.C. Circuit reversals — USTA v. FCC (net neutrality) and APSCU v. Duncan (Gainful Employment) — survived all five tightening passes at high confidence on every step.</li>
...
<ol>
  ...
  <li><strong>Dual-reviewer confirmation on Stage 1.</strong> A second, deliberately conservative classifier reviews each flagged affirmance; 22 records were dropped on disagreement.</li>
  ...
  <li><strong>Docket dedup and vacatur check.</strong> CourtListener occasionally indexes the same legal decision under two cluster IDs (observed for Mozilla v. FCC and two others); a docket-level secondary dedup collapses these. A targeted vacatur-check classifier then runs over each opinion with a reversal-coded amendment attached, asking specifically whether the court affirmed or vacated. That pass caught a Stage 1 error — <em>American Equity v. SEC</em> (2009, Rule 151A), which the classifier had coded as a step-two affirmance but which the D.C. Circuit actually vacated — and flipped it to non-affirmance, dropping its spurious "reversal" from the count.</li>
</ol>

Combined, these five passes moved the reversal count from 22 rules to 5. Subtracting the two rules that were reversed only after the paper's 2024 cutoff — Mozilla Corp v. FCC (reversed by the 2024 Safeguarding the Open Internet order) and the DOL's 2025 home-care rollback — leaves 3 within-window rules. Two of those are the paper's named reversals. The third (EPA's 2019–2020 MATS "not appropriate and necessary" flip in White Stallion v. EPA) sits on the borderline the paper's dual-review workflow is designed to resolve, and is flagged here for interest without claiming the paper's method would have counted it.
